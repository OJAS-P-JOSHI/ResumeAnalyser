...
connection = pymysql.connect(host='localhost',user='root',password='Stunner79.',db='cv')
...
def run():
    ...
    link = '[Click here to contact the Developer](https://www.linkedin.com/in/ojas-p-joshi/)'
    ...
    if choice == 'User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                """try:
                    ...
                    st.error(f"Error occurred while fetching or displaying resume video: {str(e)}")
                ...
                try:
                    ...
                    st.error(f"Error occurred while fetching or displaying interview video: {str(e)}")"""
        ...
        if st.button('Login'):
            if ad_user == 'ojas' and ad_password == '07':
                st.success("Welcome OJAS P JOSHI !")
                # Display Data
                cursor.execute('''SELECT*FROM user_data''')
                data = cursor.fetchall()
                st.header("*User's Data*")
                plot_data = pd.DataFrame(data, columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page',
                                                 'Predicted Field', 'User Level', 'Actual Skills', 'Recommended Skills',
                                                 'Recommended Course'])
                st.dataframe(plot_data)
                st.markdown(get_table_download_link(plot_data,'User_Data.csv','Download Report'), unsafe_allow_html=True)
                ## Admin Side Data
                query = 'select * from user_data;'
                plot_data = pd.read_sql(query, connection)

                ## Pie chart for predicted field recommendations
                st.markdown('''<h5 style='text-align: left; color: #021659;'>Here you can see the users details and their resumes</h5>''',
                unsafe_allow_html=True)
                ...
                df['Predicted Field'] = df['Predicted Field'].astype(str)  # Assuming 'Predicted Field' is the bytes column

                # Plot Pie Chart
                values = df['Predicted Field'].value_counts().values
                labels = df['Predicted Field'].value_counts().index
